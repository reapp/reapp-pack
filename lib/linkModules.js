...
function copyWebpackModules(opts, toDir) {
  var dependencyDir = __dirname + '/..';
  var fromDir = __dirname + '/../../..';
  var fromDirLegacy = __dirname + '/..';
  var modules = require(dependencyDir + '/package.json').dependencies;
  var npmWarningDisplayed = false;
  ...
  Object.keys(modules).forEach(function(packageName) {
    ...
    if (!fs.existsSync(srcModule)) {
      if (!npmWarningDisplayed) {
        npmWarningDisplayed = true;
        console.warn("It appears your NPM version is highly out of date. Please consider updating npm.");
      }
      srcModule = fromDirLegacy + '/node_modules/' + packageName;
    }
